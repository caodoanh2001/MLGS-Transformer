import random
from data import ImageDetectionsField, TextField, RawField, ImageDetectionsFieldIUXRay, TextFieldIUXRay, ImageDetectionsFieldMIMICCXR
from data import COCO, DataLoader, IUXray, MIMIC_CXR
import evaluation
from evaluation import PTBTokenizer, Cider, Rouge, Bleu
from models.transformer import Transformer, MemoryAugmentedEncoder, MeshedDecoder, ScaledDotProductAttentionMemory, EnsembleMultiLevelTransformerDecoder, ScaledDotProductAttention
import torch
...
def evaluate_metrics(model, dataloader, text_field):
    ...
    with tqdm(desc='Epoch %d - evaluation' % e, unit='it', total=len(dataloader)) as pbar:
        for it, (images, caps_gt) in enumerate(iter(dataloader)):
            ...
            caps_gt = [clean_report_mimic_cxr(cap_gt[0]) for cap_gt in caps_gt]
            with torch.no_grad():
                out, _ = model.beam_search(images, 100, text_field.vocab.stoi['<eos>'], 3, out_size=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--features_path', type=str, default='./mimic_cxr/')
    parser.add_argument('--annotation_folder', type=str, default='./mimic_cxr/')
    ...
    image_field_train_val = ImageDetectionsFieldMIMICCXR(detections_path=args.features_path, load_in_tmp=False, data_set='train')
    image_field_test = ImageDetectionsFieldMIMICCXR(detections_path=args.features_path, load_in_tmp=False, data_set='test')

    # Pipeline for text
    text_field = TextFieldIUXRay(init_token='<bos>', eos_token='<eos>', lower=True, tokenize='spacy',
                           remove_punctuation=False, nopoints=False, dataset='mimic_cxr')

    # Create the dataset
    dataset = MIMIC_CXR(image_field_train_val, text_field, './mimic_cxr/', args.annotation_folder, args.annotation_folder)
    train_dataset, val_dataset, _ = dataset.splits

    dataset = MIMIC_CXR(image_field_test, text_field, './mimic_cxr/', args.annotation_folder, args.annotation_folder)
    _, _, test_dataset = dataset.splits

    if not os.path.isfile('vocab/vocab_%s.pkl' % args.exp_name):
        print("Building vocabulary")
        text_field.build_vocab(train_dataset, val_dataset, min_freq=10)
    ...
    decoder = MeshedDecoder(len(text_field.vocab), 350, 3, text_field.vocab.stoi['<pad>'])
    model = Transformer(text_field.vocab.stoi['<bos>'], encoder, decoder, pretrained_backbone=args.backbone, dataset='mimic_cxr').to(device)
