...
class MeshedDecoder(Module):
    ...
    def forward(self, input, encoder_output, mask_encoder):
        ...
        for i, l in enumerate(self.layers):
            out = l(out, encoder_output, mask_queries, mask_self_attention.bool(), mask_encoder)

        out = self.fc(out)
        return F.log_softmax(out, dim=-1)

class EnsembleMultiLevelTransformerDecoder(Module):
    def __init__(self, vocab_size, max_len, N_dec, padding_idx, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, dropout=.1,
                 self_att_module=None, enc_att_module=None, self_att_module_kwargs=None, enc_att_module_kwargs=None):
        super(EnsembleMultiLevelTransformerDecoder, self).__init__()
        self.d_model = d_model
        self.word_emb = nn.Embedding(vocab_size, d_model, padding_idx=padding_idx)
        self.pos_emb = nn.Embedding.from_pretrained(sinusoid_encoding_table(max_len + 1, d_model, 0), freeze=True)
        self.layers = ModuleList(
    ...
    def forward(self, input, encoder_outputs, mask_encoders):
        # input (b_s, seq_len)
        b_s, seq_len = input.shape[:2]
        mask_queries = (input != self.padding_idx).unsqueeze(-1).float()  # (b_s, seq_len, 1)
        ...
        # Multi-level encoder outputs
        output_ensemble = []
        for encoder_output, mask_encoder in zip(encoder_outputs, mask_encoders):
            for i, l in enumerate(self.layers):
                try:
                    out = l(out, encoder_output, mask_queries, mask_self_attention.bool(), mask_encoder)
                except:
            ...
            output_ensemble.append(out_i.unsqueeze(0))
        
        self.enc_output = encoder_outputs[0]
        return torch.mean(torch.cat(output_ensemble, 0), dim=0)