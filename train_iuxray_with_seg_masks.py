import random
from data import ImageDetectionsField, TextField, RawField, ImageDetectionsFieldIUXRay, TextFieldIUXRay, ImageDetectionsFieldIUXRayAndSegmentation
from data import COCO, DataLoader, IUXray
import evaluation
from evaluation import PTBTokenizer, Cider, Rouge, Bleu
from models.transformer import Transformer, MemoryAugmentedEncoder, MeshedDecoder, ScaledDotProductAttentionMemory, ScaledDotProductAttention, EnsembleMultiLevelTransformerDecoder, TransformerWithMasks, MemoryAugmentedEncoderWithMasks
import torch
    ...
    with tqdm(desc='Epoch %d - validation' % e, unit='it', total=len(dataloader)) as pbar:
        with torch.no_grad():
            for it, (detections, captions) in enumerate(dataloader):
                ...
                images, masks, captions = images.to(device), masks.to(device), captions.to(device)

                out = model(images, masks, captions)
...
def train_xe(model, dataloader, optim, text_field):
    ...
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(dataloader)) as pbar:
        for it, (detections, captions) in enumerate(dataloader):
            ...
            loss_masks = captions_gt != 1
            # loss = compute_loss(out, captions_gt, loss_masks)
...
def train_scst(model, dataloader, optim, metric_scst, text_field):
    ...
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(dataloader)) as pbar:
        for it, (detections, caps_gt) in enumerate(dataloader):
            ...
            images = detections['images'].to(device)
            masks = detections['masks'].to(device)
            
            outs, log_probs = model.beam_search(images, masks, seq_len, text_field.vocab.stoi['<eos>'],
...
if __name__ == '__main__':
    ...
    image_field_train_val = ImageDetectionsFieldIUXRayAndSegmentation(detections_path=args.features_path, load_in_tmp=False, data_set='train', seg_path='./lungVAE/segment_lung_masks/')
    ...
    if not os.path.isfile('vocab/vocab_%s.pkl' % args.exp_name):
        ...
        text_field.build_vocab(train_dataset, val_dataset, min_freq=3)
